...
// "screenshot" for the README. Zero dependencies.
//
// Lifted from the `reins` project (assets/ansi2svg.mjs). One fix on top of it:
// `\r` now overwrites the line rather than being deleted, so progress output
// like `swift build`\'s doesn\'t concatenate into one enormous line. Worth
// porting back.
//
//   node scripts/ansi2svg.mjs <input.txt> <output.svg> ["window title"]
//
...
//
// Understands SGR codes 0/1/2/31-36 plus the 256-color and truecolor forms.
// Any other escape sequence (cursor moves, erases) that a pty capture drags
// along is silently dropped.

import * as fs from "node:fs";

const [, , inPath, outPath, title = ""] = process.argv;
if (!inPath || !outPath) {
  console.error('Usage: node scripts/ansi2svg.mjs <input.txt> <output.svg> ["title"]');
  process.exit(1);
}
...
const raw = fs
  ...
  // macOS `script` echoes the EOT that closes its stdin as the literal two
  // characters "^D" at the head of its output, so it survives the control-byte
  // filter below. Stripped at line starts only, where a capture puts it.
  .replace(/^\^D[\x08]*/gm, "")
  // pty captures leak control bytes (EOT from a closed stdin, BEL); ESC survives
  .replace(/[\x00-\x08\x0B-\x1A\x1C-\x1F]/g, "");

/** Apply carriage-return overwrite semantics. Progress output (swift build,
 *  npm, curl) redraws one line in place with `\r` and an erase-line escape;
 *  deleting the `\r`s instead of honouring them concatenates every frame of
 *  the animation into one absurdly wide line. What a terminal leaves on screen
 *  is the text after the last `\r`, so that is what gets drawn. */
